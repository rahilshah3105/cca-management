...
const StumpsContribution = () => {
  ...
  const stumpsEntries = useMemo(() => {
    const stumpsData = funds.filter(f => f.category === 'Stumps' && f.type === 'INCOME');
    
    let result = stumpsData.map((fund, index) => ({
      id: fund.id || index,
      date: fund.date,
        ...
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: '0.5rem' }}>
            Total collected: <span style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--success-color)' }}>
              ₹{stumpsEntries.reduce((sum, entry) => sum + Number(entry.amount), 0)}
            </span>
          </p>
        </div>
        {isAdmin && (
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            <button 
              ...
              onClick={() => document.getElementById('import-file-input-stumps')?.click()}
              style={{ background: 'var(--secondary-color)', border: 'none' }}
            >
              📥 Import Records
            </button>
            <input 
              id="import-file-input-stumps"
              type="file" 
