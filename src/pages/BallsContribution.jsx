...
const BallsContribution = () => {
    ...
    // Get available years from Balls contributions
    const availableYears = useMemo(() => {
        const ballsData = funds.filter(f => f.category === 'Balls' && f.type === 'INCOME');
        const years = ballsData.map(f => new Date(f.date).getFullYear());
        return [...new Set(years)].sort((a, b) => b - a);
    }, [funds]);

    // Process data - Group by player and sum amounts for Balls category
    const playerContributions = useMemo(() => {
        let ballsData = funds.filter(f => f.category === 'Balls' && f.type === 'INCOME');

        // Filter by year if selected
        if (filterYear !== 'ALL') {
            ballsData = ballsData.filter(f => new Date(f.date).getFullYear() === parseInt(filterYear));
        }

        const byPlayer = {};
        ballsData.forEach(fund => {
        ...
        return result;
    }, [funds, players, searchQuery, filterYear, sortConfig]);
